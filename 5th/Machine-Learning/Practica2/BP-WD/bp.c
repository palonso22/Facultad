...
int train(struct MLP *nn,struct DATOS *datos){
  ...
  int i,j,k;
  float h;
  float eta,suma,gamma;
  float mse,mse_train,mse_valid,mse_test,minimo_valid,weight_decay = 0.0;
  ...
  eta=nn->ETA;
  /* inicializo gamma: la nueva variable es para poder variar el learning rate durante el entrenamiento si se desea */
  gamma=nn->GAMMA;
  ...
  for (iter=1; iter<=nn->ITER; iter++) {
    ...
    for(nu1=0;nu1<datos->PR;nu1++) {
	  ...
	  for( i=1; i<=nn->N3; i++) for( j=0; j<=nn->N2; j++){
		...
		nn->w2[i][j] = nn->w2[i][j]*(1-2*gamma*eta) + nn->dw2[i][j];          /*penalización de peso + correccion*/
	  }
	  ...
	  for( j=1; j<=nn->N2; j++) for( k=0; k<=nn->N1; k++){
		...
		nn->w1[j][k] = nn->w1[j][k]*(1-2*gamma*eta) +  nn->dw1[j][k];      /*penalización de peso + correccion*/
	  }
    ...
    if ((iter % nn->NERROR) == 0) {
      ...
      /*Calcular suma total de pesos al cuadrado y Multiplicar por gamma*/
      weight_decay = 0.0;
      /*Sumar pesos capa 3*/
      for(i=1;i<=nn->N3;i++)
         for(j=0;j<=nn->N2;j++)
           weight_decay += pow(nn->w2[i][j],2);

       /*Sumar pesos capa 2*/
       for(i=1;i<=nn->N2;i++)
          for(j=0;j<=nn->N1;j++)
            weight_decay += pow(nn->w1[i][j],2);

       /*Multiplicar por gamma*/
       weight_decay *= gamma;

      /*guardo todos los errores en el archivo .mse*/
      fprintf(ferror,"%f\t%f\t%.12f\t%f\t",mse,mse_train,weight_decay,mse_test);
