...
/*clases para el problema de iteraciones*/
#define I1 0.1
#define I2 0.1 + 1/3
#define I3 0.1 + 2/3
...
int read_data(char *filenamepasado,struct DATOS *datos){
  ...
  for(k=0;k<datos->PTOT;k++){
	 ...
	 datos->data[k][0]=-1.0;
 	 for(i=1;i<=datos->N1+datos->N3;i++){
	   fscanf(fpat,"%f",&valor);
     if(i == datos->N1+1)datos->data[k][i]=0.1+1.0/3.0*valor;
     else datos->data[k][i]=valor;
  ...
  for(k=0;k<datos->PTEST;k++){
	 ...
	 datos->test[k][0]=-1.0;
         for(i=1;i<=datos->N1+datos->N3;i++){
           fscanf(fpat,"%f",&valor);
           if(i == datos->N1+1)datos->test[k][i]=0.1+1.0/3.0*valor;
           else datos->test[k][i]=valor;
  ...
  *discrete_error=*mse=0.0;
  ...
  for (patron=pat_ini; patron<pat_fin; patron++) {
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    // La predicción es el techo de la salida de la neurona
    float pred = clasificar2(nn->x3[1]);
    printf("pred:%f, valor real:%f\n",pred,S[nu][nn->N1+1]);
    if(pred != S[nu][nn->N1+1]) *discrete_error += 1.0;
...
int train(struct MLP *nn,struct DATOS *datos){
  ...
  for (iter=1; iter<=nn->ITER; iter++) {
    ...
    for(nu1=0;nu1<datos->PR;nu1++) {
	  ...
	  for( i=1; i<=nn->N3; i++)
		    nn->grad3[i] = nn->x3[i]*(1-nn->x3[i])*(nn->target[i] - nn->x3[i]);  /*corresponde a sigmoidea*/
