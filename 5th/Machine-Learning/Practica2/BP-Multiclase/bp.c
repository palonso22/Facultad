...
int read_data(char *filenamepasado,struct DATOS *datos){
  ...
  for(k=0;k<datos->PTOT;k++){
	 ...
	 datos->data[k][0]=-1.0;
 	 ...
 	 for(i=1;i<=datos->N1+1;i++){
	   fscanf(fpat,"%f",&valor);
	   if (i < datos->N1+1) datos->data[k][i]=valor/255;
	   if(CONTROL>1 && i < datos->N1+1) printf("%f\t",datos->data[k][i]);
	   separador=getc(fpat);
	   if(separador!=',') ungetc(separador,fpat);
  	 }
    if (valor+1 > datos->N3 || valor < 0){
      printf("Error: valor de clase fuera de rango\n");
      return 1;
    }

    //Le asignamos 1 a la neurona que deba reaccionar
    int prueba;
    for(i=datos->N1+1; i<=datos->N1+datos->N3;i++)
          datos->data[k][i] = 0;
    datos->data[k][datos->N1+1+(int)valor] = 1;
  ...
  for(k=0;k<datos->PTEST;k++){
   ...
   for(i=1;i<=datos->N1+1;i++){
       fscanf(fpat,"%f",&valor);
       if(i < datos->N1+1) datos->test[k][i]=valor/255;
   ...
   for(i=datos->N1+1; i <= datos->N1+datos->N3;i++)
       datos->test[k][i] = 0;
   datos->test[k][datos->N1+1+(int)valor] = 1;
...
/*evaluar: calcula los valores de salida de la red para un conjunto de datos
  la matriz S es alguno de los datos validos1937012526
  ...
  *discrete_error=*mse=0.0;
  ...
  for (patron=pat_ini; patron<pat_fin; patron++) {
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    int pred = 1,claseReal=-1;
    for(i = 1; i <= nn->N3; i++) {
        *mse += (nn->x3[i]-S[nu][nn->N1+i])*(nn->x3[i]-S[nu][nn->N1+i]);                           /*mse se calcula para todas las salidas*/
        if (fabs(1-nn->x3[i]) < fabs(1-nn->x3[pred])) pred = i; // La predicción de la red será la que dé un output más cercano al 1
        if (S[nu][nn->N1+i] == 1) claseReal = i; // Guardar cuál es la neurona que debería reaccionar
    }

    if (pred != claseReal) *discrete_error += 1.0;
...
int train(struct MLP *nn,struct DATOS *datos){
  ...
  for (iter=1; iter<=nn->ITER; iter++) {
    ...
    for(nu1=0;nu1<datos->PR;nu1++) {
	  ...
	  for( i=1 ;i<=nn->N3; i++) mse += (nn->x3[i]-nn->target[i]) * (nn->x3[i]-nn->target[i]);
