...
/* ---------------------------------------------------------------------------------- */
int arquitec(char *filename,struct MLP *nn,struct DATOS *datos,int ctosData,int ctosTrain, int ctosTest){
  FILE *b;
  char filepat[100];
  int i,j;
  /*bandera de error*/
...
int main(int argc, char **argv){
  ...
  error=arquitec(argv[1],&nn,&datos,atoi(argv[2]),atoi(argv[3]),atoi(argv[4]));
  if(error){
