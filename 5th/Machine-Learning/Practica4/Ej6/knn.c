...
/*defino una estructura para el clasificador knn*/
struct KNN {
...
struct DATOS {
    ...
    float *pred;                        /* valor real predecido*/
...
/* ------------------------------------------------------------------- */
float prob(struct KNN *nb,float x,int feature,int clase)  {
...
/* ------------------------------------------------------------------------------ */
/*Calcular el valor real de input encontrando los vecinos más cercanos a una dist DIST MAX y haciendo una votación entre estos
para predecir la clase de input
/* ------------------------------------------------------------------------------ */
float output(struct KNN *knn,float *input,struct DATOS* datos){
  int i;
  ...
  for(i=0; i<ctos; i++){
    ...
    distancias[i][1] = datos->data[index][knn->N_IN];
  }
  ...
  unsigned d = 0;
  if (distancias[0][0] == 0) distancias[0][0] = pow(10,-5);   //Si se encuentra el mismo punto, en vez de 0 le ponemos una distancia muy chica
                                                             // para evitar la división por 0 al pesarla

  float weight, dividendo = 0,divisor = 0;
  for(i=0;i<knn->k;i++){
    weight = pow(distancias[i+d][0],-2); //Calcular peso
    dividendo += weight*distancias[i+d][1]; // Sumar distancia pesada * valor real
    divisor += weight; // Sumar peso
  }
...
float evaluar(struct KNN *knn,struct DATOS *datos,float **S,int pat_ini,int pat_fin,int usar_seq){
  ...
  for (patron=pat_ini; patron < pat_fin; patron ++) {
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    /*estimar error*/
    mse += pow(S[nu][knn->N_IN]-datos->pred[nu],2);
  }
...
int hacerEvaluaciones(struct KNN *knn,struct DATOS *datos){
  ...
  int error;
  int N_TOTAL;
  float train_error,valid_error,test_error,minValidError=1.0E20;;
  FILE *fpredic;
  char filepat[100];
  unsigned bestK = 0; //Mejor valor para knn q encontremos
 ...
 for(i=1;i<=nroEvaluaciones;i++){
    ...
    fprintf(ferror,"%d %f %f %f\n",i,train_error,valid_error,test_error);
