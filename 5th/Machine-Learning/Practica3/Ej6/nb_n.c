...
int train(struct NB *nb,struct DATOS *datos){
  ...
  FILE *fpredic, *ferror;
  char filepat[100];
  unsigned class, index,valDiscreto,nroBins;
  ...
  for(i = 0;i<N_TOTAL;i++){
    ...
    nb->probAPriori[class]+= 1.0/(float)N_TOTAL;
  }

    //Segunda parte: crear árboles discretos
  DRecursivo d;
  Arbol* a;
  Vector* v;
  ...
  for(i = 0;i<nb->N_IN;i++){
    ...
    for(j=0; j <nroBins;j++) nb->probAPosteriori[i][j] = (float*) calloc(nb->N_Class,sizeof(float));
    nb->vectoresDiscretos[i] = v;
  }


  //Tercera parte: Calcular probabilidad a posteriori
  float P = pow(5,-1); // Probabilidad de los ejemplos virtuales
  /*Calcular probabilidad a posteriori*/
  for(i = 0; i < N_TOTAL;i++){
    index = datos->seq[i];
    class = datos->data[index][nb->N_IN]; // Guardar clase
    // Calculamos probilidades a posteriori para los atributos
    for(j = 0; j < nb->N_IN;j++){
      valDiscreto = calcularValorDiscreto(nb->vectoresDiscretos[j],datos->data[index][j]);
      frecClass = nb->probAPriori[class]*N_TOTAL;
      /* 1er parte: asignar  1/(frecuencia de la clase + M)*/
      nb->probAPosteriori[j][valDiscreto][class] += pow(frecClass+M,-1);
    }
  }
  // Cuarta parte: Sumar correción
  for(i=0;i<nb->N_IN;i++){
    nroBins = nb->vectoresDiscretos[i]->n;
    for(j=0;j<nroBins;j++)
      for(k=0;k<nb->N_Class;k++){
          frecClass = nb->probAPriori[class]*N_TOTAL;
          nb->probAPosteriori[i][j][k] += ( M*P/(frecClass+M) );
      }
  }
