...
int define_matrix_nb(struct NB *nb){
  ...
  for(;i < nb->N_IN;i++){
    ...
    for(j=0; j < nb->BINS;j++) nb->probAPosteriori[i][j] = (float*) calloc(nb->N_Class,sizeof(float));
  }
...
int train(struct NB *nb,struct DATOS *datos){
  ...
  FILE *fpredic, *ferror;
  char filepat[100];
  unsigned class, index,valDiscreto,minNroBins;
  ...
  for(int nu = 1;nu<=cantBins;nu++){
    ...
    for(i = 0; i < N_TOTAL;i++){
      ...
      for(j = 0; j < nb->N_IN;j++){
        valDiscreto = discretizar(datos->data[index][j],nb->BINS);
    ...
    for(i=0;i<nb->N_IN;i++)
      for(j=0;j<nb->BINS;j++)
        for(k=0;k<nb->N_Class;k++){
            frecClass = nb->probAPriori[class]*N_TOTAL;
            /*2da parte : sumar MP/(frecuencia de la clase + M), luego p(ajk/ci)= (nc+PM)/(n+M)*/
