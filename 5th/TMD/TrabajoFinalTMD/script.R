...
kable(resKmeans(cleanData[,-281],cleanData[,281]),caption="Kmeans con k = 6 y 280 features",align = rep("r",4),format="latex") %>%
...
kable(resKmeans(cleanData[,-51],cleanData[,51]),caption="Kmeans con k = 6 y 50 features",align = rep("r",4),format="latex") %>%
kable_styling(latex_options = "HOLD_position") %>%
column_spec(1:7,background = "#FFFF19") %>%
column_spec(1, background  = "#FF8000") %>%
row_spec(0,background = "#FF6619")
...
cleanData <-  data[,c(res$ordered.features.list[1:280],562)]
kable(resHClust(cleanData[,-281],cleanData[,281]),caption="Hclust con 280 features",align = rep("r",4),format="latex") %>%
kable_styling(latex_options = "HOLD_position") %>%
column_spec(1:7,background = "#FFFF19") %>%
column_spec(1, background  = "#FF8000") %>%
row_spec(0,background = "#FF6619")
...
cleanData <-  data[,c(res$ordered.features.list[1:50],562)]
kable(resHClust(cleanData[,-51],cleanData[,51]),caption="HClust con 50 features",align = rep("r",4),format="latex") %>%
kable_styling(latex_options = "HOLD_position") %>%
column_spec(1:7,background = "#FFFF19") %>%
column_spec(1, background  = "#FF8000") %>%
row_spec(0,background = "#FF6619")
...
cleanData <-  data[,res$ordered.features.list[1:280]]
evalEstabilidad(cleanData,"estabilidad")
...
#Neural networks (no lo pude usar correctamente a veces tira error)
neuralNetwork <- function(train,trainClass,test,testClass,formula){
  ...
  model <- neuralnet(form,data=train2, hidden=5,act.fct = "logistic", linear.output = FALSE,threshold = 0.1)
...
#numero de variables que vamos a considerar
n<-561
# usamos distintos metodos para clasificar los datos
subdata<-data[,c(res$ordered.features.list[1:n],562)]
subdata[,n+1]<-as.factor(subdata[,n+1])
resRf <- testPerformance(subdata,rf,5,subdata[,n+1],Activity~.)
print(resRf)
resPoly <-testPerformance(subdata,polyKernel,5,subdata[,n+1],Activity~.)
print(resPoly)
