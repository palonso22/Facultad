...
BSTree bstree_max(BSTree arbol){
	if(!arbol)return NULL;
	for(;arbol->right!=NULL;arbol=arbol->right);
	return arbol;
	}

BSTree bstree_balancear(BSTree arbol){
	if(!arbol)return NULL;
	//Calcular la cantidad de nodos en ambos subarboles
	int cantElementsIzq=bstree_nelementos(arbol->left),cantElementsDer=bstree_nelementos(arbol->right);
	if(fabs(cantElementsIzq-cantElementsDer)>1){
		BSTree temp;
		if(cantElementsIzq>cantElementsDer){
			BSTree max=bstree_max(arbol->left);
			temp=arbol->left;
			arbol->left=NULL;
			max->right=arbol;
			return bstree_balancear(temp);
			}
		BSTree min=bstree_min(arbol->right);
		temp=arbol->right;
		arbol->right=NULL;
		min->left=arbol;
		return bstree_balancear(temp);;
