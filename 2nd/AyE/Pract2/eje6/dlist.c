...
SList dlist_agregar_final(SList lista, int dato) {
  ...
  if (!lista){
    nuevoNodo->movimiento=(uintptr_t)NULL;
    return nuevoNodo;
  }
  SList aux,nodo=lista,ant=NULL;
  for(;(SList)(nodo->movimiento^(uintptr_t)ant) != NULL;){
  aux=nodo;
  nodo =(SList)((uintptr_t)ant^nodo->movimiento);
  ant=aux;
}
  nuevoNodo->movimiento=(uintptr_t)nodo^(uintptr_t)NULL;
  nodo->movimiento=(uintptr_t)ant^(uintptr_t)nuevoNodo;
  return lista;
}
...
SList dlist_agregar_inicio(SList lista, int dato) {
  ...
  nuevoNodo->movimiento=(uintptr_t)NULL^(uintptr_t)lista;
  lista->movimiento=(uintptr_t)nuevoNodo^(uintptr_t)lista->movimiento;
  return nuevoNodo;
}
...
void dlist_recorrer(SList lista, FuncionVisitante visit) {
  ...
  SNodo* ant=NULL;
  SNodo* aux;
  for (SNodo *nodo =lista; nodo != NULL;){
    aux=nodo;
    visit(nodo->dato);
    nodo=(SNodo*)(nodo->movimiento^(uintptr_t)ant);
    ant=aux;
  }
}
