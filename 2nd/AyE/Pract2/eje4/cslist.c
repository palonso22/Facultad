...
void cslist_destruir(SList lista) {
  if(!lista)return;
  SNodo *nodoAEliminar;
  SNodo *aux=lista;
  while (lista->sig!=aux) {
...
SList cslist_agregar_final(SList lista, int dato) {
...
SList cslist_agregar_inicio(SList lista, int dato) {
  SNodo *nuevoNodo = malloc(sizeof(SNodo));
  nuevoNodo->dato = dato;
  if(!lista){
    nuevoNodo->sig=nuevoNodo;
...
void cslist_recorrer(SList lista, FuncionVisitante visit) {
  if(!lista){
    printf("Lista Vacia");
    return;
  }
...
int cslist_longitud(SList lista){
  int i=0;
  if(!lista)return i;
  SList aux=lista;
...
void cslist_concatenar(SList lista, SList lista2){
  if(!lista || !lista2)return ;
  SList aux=lista;
  for(;lista->sig!=aux;lista=lista->sig);
  lista->sig=lista2;
  SList aux2=lista2;
  for(;lista2->sig!=aux2;lista2=lista2->sig);
  lista2->sig=aux;
}
