...
float** MatrizInversa(int** Matriz,int**MAdjunta,int CantFilas,int CantColumnas);
...
int main(){
	...
	int **ptrMatriz=malloc(sizeof(int*)*3),**MCofactores,**MAdjunta;
	float **MInversa;
	...
	ImprimirMatrizInversa(MInversa,3,3);
}
...
float** MatrizInversa(int** Matriz,int**MAdjunta,int CantFilas,int CantColumnas){
	...
	float **MInversa=crearMatrizInversa(CantFilas,CantColumnas),determinante;
	int i,j;
	determinante=CalcDeterminante(Matriz,CantFilas,CantColumnas);
	for(i=0;i<CantFilas;i++){
		for(j=0;j<CantColumnas;j++){
			MInversa[i][j]=pow(determinante,-1)*(float)MAdjunta[i][j];
		}
	}
	return MInversa;
}
