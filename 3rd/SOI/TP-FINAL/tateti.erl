...
chequeoRango(Fil, Col)->
  if (Fil > 0) and (Fil < 4) and (Col > 0) and (Col < 4) -> true;
    true -> false
  end.
...
tateti(Tablero,Jugadores,Observadores,TurnoDe)->
  receive
    ...
    {solicito_jugar,Usuario,Fil,Col} ->
      C = chequeoRango(Fil, Col),
      if not C ->
        Usuario#usuario.psocket ! {reenviar, "Posicion incorrecta"++n()},
        tateti(Tablero,Jugadores,Observadores,TurnoDe);
        true ->
            ...
            if B2  ->
              ...
              if 
                ...
                B4 ->
                  ...
                  map(fun(_,ObsSocket) -> ObsSocket ! {actualizar,quitoObs,self()} end, Observadores),
                  PsocketUsuario ! {reenviar, "Hubo empate! Llevas "++integer_to_list(Usuario#usuario.ganadas)++" ganadas, "++integer_to_list(Usuario#usuario.perdidas)++" perdidas y "++integer_to_list(Usuario#usuario.empates+1)++" empatadas."++n()},
                  PsocketUsuario ! {actualizar,empates,0},
                  PsocketUsuario ! {actualizar,jugando,undefined},
                  PsocketJugador2 ! {reenviar, "Hubo empate! Llevas "++integer_to_list(Jugador2#usuario.ganadas)++" ganadas, "++integer_to_list(Jugador2#usuario.perdidas)++" perdidas y "++integer_to_list(Usuario#usuario.empates+1)++" empatadas."++n()},
                  PsocketJugador2 ! {actualizar,empates,0},
                  PsocketJugador2 ! {actualizar,jugando,undefined},
                  partidas ! {cerro_ini, self()},
                  ok;
                true ->
    ...
    {se_va_jugador,Jugador} ->
      ...
      case length(Jugadores) of
        1 ->
          Msg = format("Te fuiste de la partida. Llevas ~p ganadas, ~p perdidas y  ~p empatadas.",[Jugador#usuario.ganadas,Jugador#usuario.perdidas,Jugador#usuario.empates]),
          partidas ! {cerro_espera, self()};
        2 ->
          Msg = format("Te fuiste de la partida. Llevas ~p ganadas, ~p perdidas y  ~p empatadas.",[Jugador#usuario.ganadas,Jugador#usuario.perdidas+1,Jugador#usuario.empates]),
          ...
          Msg2 = format("~p se fue. Ganaste la partida! Llevas ~p ganadas, ~p perdidas y ~p empates.",[NombreJugador,ElOtroJugador#usuario.ganadas+1,ElOtroJugador#usuario.perdidas,Jugador#usuario.empates]),
      ...
      Jugador#usuario.psocket ! {actualizar,jugando,undefined}, %% Setear jugando a undefined
